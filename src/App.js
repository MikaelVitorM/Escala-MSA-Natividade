...
const dbGet    = (t, qs = "")   => sb(`${t}?${qs}`);
const dbInsert = (t, body)      => sb(t, { method: "POST", body: JSON.stringify(body) });
const dbUpsert = (t, body)      => sb(t, { method: "POST", headers: { Prefer: "resolution=merge-duplicates,return=representation" }, body: JSON.stringify(body) });
const dbDelete = (t, qs)        => sb(`${t}?${qs}`, { method: "DELETE" });

const IGREJAS = [
  "Paroquia Natividade - Presbitério",
  "Paroquia Natividade - Capela do Santíssimo",
...
function getFuncoesNecessarias(dia) {
  ...
  if (dia.tem_incenso || dia.temIncenso) {
    funcs.find(f => f.nome === "Ceroferário").qtd = 6;
    funcs.push({ nome: "Turiferário", qtd: 1 }, { nome: "Naveteiro", qtd: 1 });
  } else if (dom) {
    funcs.find(f => f.nome === "Ceroferário").qtd = 6;
  }
  ...
  if (!nome) return (
    <div style={{ ...bgApp, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24 }}>
      <div style={{ textAlign: "center", marginBottom: 24 }}>
        <div style={{ fontSize: 32, color: C.ouro }}>✦</div>
        ...
        <p style={{ color: C.ouroPale, fontSize: 13, margin: 0 }}>Selecione seu nome para marcar disponibilidade</p>
      </div>
  ...
  return (
    <div style={{ ...bgApp, paddingBottom: 48 }}>
      ...
      <div style={{ maxWidth: 640, margin: "0 auto", padding: "18px 14px" }}>
        <p style={{ color: C.ouroPale, fontSize: 13, marginBottom: 14 }}>Marque os dias em que você está disponível:</p>
        ...
        {dias.length > 0 && <button onClick={confirmar} disabled={salvando} style={{ ...btnPri, width: "100%", marginTop: 8, padding: 13, fontSize: 14, opacity: salvando ? .7 : 1 }}>{salvando ? "⏳ Salvando…" : `✓ Confirmar (${total} dia${total !== 1 ? "s" : ""})`}</button>}
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  useEffect(() => {
    carregarDados(true);
    const intervalo = setInterval(() => carregarDados(false), 15000);
...
await carregarDados(true);
  ...
  const gerarSorteio = () => { setSorteioAtual(sortearEscala(dias, membros, disponibilidades, historico)); setView(4); };
        ...
        {view === 2 && (
          <div>
            <Card style={{ marginBottom: 14, textAlign: "center" }}>
              ...
              <p style={{ color: `${C.ouro}70`, fontSize: 11, marginTop: 10 }}>Envie este link no grupo do WhatsApp. Atualiza automaticamente a cada 15 segundos.</p>
            </Card>
            <Card>
              ...
              {dias.map(dia => {
                ...
                return (
                  <div key={dia.id} style={{ marginBottom: 12, paddingBottom: 12, borderBottom: `1px solid ${C.ouro}20` }}>
                    ...
                    {!resp.length ? <span style={{ color: `${C.ouro}50`, fontSize: 11 }}>Sem respostas ainda</span>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <Card>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(150px,1fr))", gap: 10, marginBottom: 14 }}>
                ...
                <div style={{ background: "rgba(0,0,0,0.2)", borderRadius: 8, padding: 12, textAlign: "center" }}>
                  <div style={{ fontSize: 13, color: C.ouroClaro, fontWeight: 700, textTransform: "capitalize" }}>{statsPre.maisDia ? `${fmtData(statsPre.maisDia.data).split(",")[0]} ${statsPre.maisDia.horario}` : "—"}</div>
                  <div style={{ fontSize: 11, color: C.ouroPale }}>+ disponível ({statsPre.maisDia?.count || 0})</div>
                </div>
                <div style={{ background: "rgba(0,0,0,0.2)", borderRadius: 8, padding: 12, textAlign: "center" }}>
                  <div style={{ fontSize: 13, color: "#ff9966", fontWeight: 700, textTransform: "capitalize" }}>{statsPre.menosDia ? `${fmtData(statsPre.menosDia.data).split(",")[0]} ${statsPre.menosDia.horario}` : "—"}</div>
                    ...
                    {dia.funcs.map(({ nome: fn, qtd }) => {
                      ...
                      return (
                        <div key={fn} style={{ background: f > 0 ? "rgba(180,40,0,0.2)" : `${C.ouro}08`, border: `1px solid ${f > 0 ? "#ff440035" : C.ouro + "35"}`, borderRadius: 7, padding: "8px 10px" }}>
                          ...
                          {pessoas.map(p => { const mb = membros.find(m => m.nome === p); return (<div key={p} style={{ fontSize: 11, color: C.ouroPale, marginBottom: 2, display: "flex", justifyContent: "space-between" }}><span>✓ {p.split(" ").slice(0, 2).join(" ")}</span>{mb?.score > 0 && <span style={{ color: C.ouro, fontSize: 10 }}>★{mb.score}</span>}</div>); })}
